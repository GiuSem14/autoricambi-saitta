"use client";

import { useState, FormEvent } from "react";
import { WHATSAPP_NUMBER } from "@/lib/constants";

interface FormData {
  marca: string;
  modello: string;
  anno: string;
  motorizzazione: string;
  targa: string;
  messaggio: string;
}

const initialState: FormData = {
  marca: "",
  modello: "",
  anno: "",
  motorizzazione: "",
  targa: "",
  messaggio: "",
};

export default function RicambioForm() {
  const [form, setForm] = useState<FormData>(initialState);
  const [errors, setErrors] = useState<Partial<FormData>>({});

  function validate(): boolean {
    const newErrors: Partial<FormData> = {};
    if (!form.marca.trim()) newErrors.marca = "Campo obbligatorio";
    if (!form.modello.trim()) newErrors.modello = "Campo obbligatorio";
    if (!form.anno.trim()) {
      newErrors.anno = "Campo obbligatorio";
    } else if (!/^\d{4}$/.test(form.anno) || +form.anno < 1900 || +form.anno > new Date().getFullYear() + 1) {
      newErrors.anno = "Anno non valido";
    }
    if (!form.targa.trim()) newErrors.targa = "La targa è obbligatoria";
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  }

  function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (!validate()) return;

    // Traccia invio form su GA4 per misurare conversioni
    if (typeof window !== "undefined" && window.gtag) {
      window.gtag("event", "form_submit", {
        form_name: "richiesta_ricambio",
        targa: form.targa ? "compilata" : "vuota",
        motorizzazione: form.motorizzazione ? "compilata" : "vuota",
        messaggio: form.messaggio ? "compilato" : "vuoto",
        // NON inviare dati personali come nome, telefono, email
      });
    }

    const lines = [
      `Ciao, cerco un ricambio per:`,
      `Marca: ${form.marca}`,
      `Modello: ${form.modello}`,
      `Anno: ${form.anno}`,
      form.motorizzazione ? `Motorizzazione: ${form.motorizzazione}` : null,
      form.targa ? `Targa: ${form.targa}` : null,
      form.messaggio ? `\nNote: ${form.messaggio}` : null,
    ]
      .filter(Boolean)
      .join("\n");

    const encoded = encodeURIComponent(lines);
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encoded}`, "_blank");
  }

  function handleChange(field: keyof FormData, value: string) {
    setForm((prev) => ({ ...prev, [field]: value }));
    if (errors[field]) setErrors((prev) => ({ ...prev, [field]: undefined }));
  }

  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <Field
          label="Marca *"
          placeholder="es. Fiat, Volkswagen, Renault"
          value={form.marca}
          error={errors.marca}
          onChange={(v) => handleChange("marca", v)}
        />
        <Field
          label="Modello *"
          placeholder="es. Punto, Golf, Clio"
          value={form.modello}
          error={errors.modello}
          onChange={(v) => handleChange("modello", v)}
        />
        <Field
          label="Anno *"
          placeholder="es. 2015"
          value={form.anno}
          error={errors.anno}
          onChange={(v) => handleChange("anno", v)}
          type="number"
        />
        <Field
          label="Motorizzazione"
          placeholder="es. 1.4 TDI, 2.0 HDI"
          value={form.motorizzazione}
          onChange={(v) => handleChange("motorizzazione", v)}
        />
        <Field
          label="Targa *"
          placeholder="es. AB123CD"
          value={form.targa}
          error={errors.targa}
          onChange={(v) => handleChange("targa", v)}
        />
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-300 mb-1">
          Messaggio / Ricambio cercato
        </label>
        <textarea
          rows={4}
          placeholder="Descrivi il ricambio che cerchi o eventuali dettagli aggiuntivi..."
          value={form.messaggio}
          onChange={(e) => handleChange("messaggio", e.target.value)}
          className="w-full bg-[#111111] border border-gray-700 rounded-lg px-4 py-3 text-brand-white placeholder-gray-600 focus:outline-none focus:border-brand-yellow transition-colors text-sm resize-none"
        />
      </div>

      <button
        type="submit"
        className="w-full sm:w-auto flex items-center justify-center gap-3 bg-[#25D366] hover:bg-green-500 text-white font-bold px-8 py-4 rounded-lg transition-colors text-base"
      >
        <WhatsAppIcon />
        Invia richiesta su WhatsApp
      </button>
    </form>
  );
}

interface FieldProps {
  label: string;
  placeholder: string;
  value: string;
  error?: string;
  type?: string;
  onChange: (v: string) => void;
}

function Field({ label, placeholder, value, error, type = "text", onChange }: FieldProps) {
  return (
    <div>
      <label className="block text-sm font-medium text-gray-300 mb-1">{label}</label>
      <input
        type={type}
        placeholder={placeholder}
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className={`w-full bg-[#111111] border rounded-lg px-4 py-3 text-brand-white placeholder-gray-600 focus:outline-none focus:border-brand-yellow transition-colors text-sm ${
          error ? "border-red-500" : "border-gray-700"
        }`}
      />
      {error && <p className="mt-1 text-xs text-red-400">{error}</p>}
    </div>
  );
}

function WhatsAppIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
  );
}
